...
const Signup = () => {
  ...
  // Updates field value and clears error message when user types
  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData((prev) => ({
      ...prev,
  ...
  // Determine the password strength score (0 to 4)
  const getPasswordStrength = (password) => {
    if (!password) return 0;
    let strength = 0;
    if (password.length >= 8) strength++;
  ...
  // Map strength to a corresponding color
  const getPasswordStrengthColor = (password) => {
    const strength = getPasswordStrength(password);
    switch (strength) {
      case 1:
  ...
  // Map strength score to text feedback
  const getPasswordStrengthText = (password) => {
    const strength = getPasswordStrength(password);
    switch (strength) {
      case 0:
